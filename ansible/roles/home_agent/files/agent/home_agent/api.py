...
def read_secret(path: str, label: str = "API key") -> str:
    value = Path(path).read_text(encoding="utf-8").strip()
    if len(value) < 20 or value == "CHANGE_ME":
        raise RuntimeError(f"{label} file is not configured")
    return value
...
def create_provider() -> AnthropicMessagesProvider:
    ...
    return AnthropicMessagesProvider(
        api_key=read_secret(key_path, "Anthropic API key"),
        model=model,
...
def create_transcriber() -> OpenAIAudioTranscriber:
    key_path = os.environ.get("OPENAI_API_KEY_FILE", "/run/secrets/openai/openai_api_key")
    model = os.environ.get("HOME_AGENT_STT_MODEL", "whisper-1")
    return OpenAIAudioTranscriber(api_key=read_secret(key_path, "OpenAI API key"), model=model)
